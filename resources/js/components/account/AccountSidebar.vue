...
const roleLabel = computed(() => {
  ...
  if (authStore.role === 'sales_director') return 'Director Vânzări';
  if (authStore.role === 'sales_agent') return 'Agent Vânzări';
...
onMounted(async () => {
    if (authStore.token && (!authStore.user || !authStore.user.roles)) {
        await authStore.refreshUser();
    }
    console.log('Sidebar mounted. User:', authStore.user, 'Role:', authStore.role);
});
